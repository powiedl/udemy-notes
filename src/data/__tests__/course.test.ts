...
describe('getTrainerSuggestionsLogic', () => {
  ...
  it('Filtert, trimmt und begrenzt die Trainer-Vorschläge auf max. 5 Unikate', async () => {
    const mockSuggestions = [
      { trainer: 'Max Mustermann' },
      { trainer: 'max mustermann' }, // Duplikat (Case-Insensitive)
      { trainer: ' Erika Musterfrau ' }, // Muss getrimmt werden
      { trainer: 'John Doe' },
      { trainer: 'Jane Doe' },
      { trainer: 'Alice' },
      { trainer: 'Bob' }, // Sollte abgeschnitten werden (Nr. 6)
    ]
